...
export class MusicList<Thing> extends React.Component<PublicProps<Thing>> {
  public render() {
    ...
    const overflow = this.props.fit === 'content' ? '' : 'auto';
    return (
      <div style={{ overflow, padding: '30px' }}>
        <Heading level="2">{this.props.title}</Heading>
        <TileList gutter={15} tileWidth={160}>
          {items}
        </TileList>
        <Box justify="center" align="center" margin="20px">
          {this.props.hasMore && <LoadMoreButton onMore={this.props.onMore} loading={this.props.loading} />}
