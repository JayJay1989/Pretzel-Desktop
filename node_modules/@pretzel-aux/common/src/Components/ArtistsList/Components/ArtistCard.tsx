...
export class ArtistCardPresentation extends React.Component<Props> {
  public render() {
    ...
    const art = (
      <Image fit="cover" src={toUrl(artist.artworkGuid, ArtworkSize.Medium)} style={{ borderRadius: '50%' }} width="100%" />
    );
    const text = (
      <>
        ...
        <Box flex="grow" justify="end">
          <Text size="10px" color={Night.textMuted} margin={{ top: '16px' }} align-self="end">
            {artist.stats.totalCount} Songs&nbsp;&nbsp;&nbsp;•&nbsp;&nbsp;&nbsp;{minutesOrHours(artist.stats.duration)}
          </Text>
        </Box>
      </>
    );
    return <MusicCard art={art} text={text} circle={true} active={isPlaying} onClick={this.handleClick} />;
    // return <MusicCard art={art} text={text} circle={true} active={isPlaying} onClick={this.handleClick} linkTo={`/segment/${artist.id}`} />;
  }
  private handleClick = () => {
