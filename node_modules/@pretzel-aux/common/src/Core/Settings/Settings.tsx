...
import { GetSettingsQuery, SetSettingsInput, SetSettingsMutation, Settings } from './Queries/schema';
...
const GetSettings = loader('./Queries/get-settings.graphql');
const SetSettings = loader('./Queries/set-settings.graphql');
...
interface GraphQLProps {
  data: GraphqlQueryControls & GetSettingsQuery;
  setSettings: Mutation<SetSettingsInput, SetSettingsMutation>;
}
...
export class SettingsProviderImplementation extends React.Component<Props, SettingsContext> {
  ...
  private async getSettingsFromPlatform() {
    try {
      ...
      this.setState({ [SettingsKeys.AudioOutputDevice]: audioOutputDevice });
    } catch (e) {
      console.log('Could not load Audio Output Device: ', e);
    }
  }
}
...
export const SettingsProvider = compose(
  ...
  graphql<{}, GetSettingsQuery>(GetSettings),
  withMutation(SetSettings, { name: 'setSettings' }),
