...
import { compose, graphql, GraphqlQueryControls, withApollo, withMutation } from 'react-apollo';
import { ApolloClient } from 'apollo-client';
import {
  BasicTrack,
  CurrentPlayTokenQuery,
  NextPlayTokenMutation,
  NotifyPlayTokenInput,
  NotifyPlayTokenMutation,
  PlayToken,
  Segment,
  SetActiveSegmentInput,
  SetActiveSegmentMutation,
} from './Queries/schema';
...
interface GrqphQLProps {
  data: GraphqlQueryControls & CurrentPlayTokenQuery;
  setActiveSegment: Mutation<SetActiveSegmentInput, SetActiveSegmentMutation>;
}
...
export interface PlayerContext {
  ...
  // Actions / inputs
  playSegment: (segment: SetActiveSegmentInput) => void;
...
export const { Provider:BasePlayerProvider, Consumer: PlayerConsumer, withContext: withPlayerContext } = createContext<PlayerContext>(
  'VerticalPlayer',
  {
    playToken: null,
    activeSegment: null,
...
class PlayerProviderImplementation extends React.Component<Props, PlayerContext> {
  state: PlayerContext = {
    ...
    playSegment: (segment: SetActiveSegmentInput) => this.playSegment(segment),
    changeSegment: (id: string) => this.changeSegment(id),
    playTrack: (track: BasicTrack) => this.playTrack(track),
    ...
    sendAnalyticsEvent: (type: AnalyticsEventType, playToken?: PlayToken | null, source?: string) =>
  ...
  private playSegment(segment: SetActiveSegmentInput) {
    if (this.state.activeSegment && this.state.activeSegment.id === segment.id) return;
    console.debug('Playing Segment', segment);
    this.props
  ...
  private playTrack(track: BasicTrack) {
  ...
  sendAnalyticsEvent = (type: AnalyticsEventType, playToken?: PlayToken | null, source: string = 'player') => {
    if (!playToken) {
  ...
  sendNotify = (token: string) => {
    ...
    return this.props.client.mutate<NotifyPlayTokenMutation, NotifyPlayTokenInput>({
  ...
  private getNext(trackId?: string): Promise<PlayToken> {
    this.setState({ loadingNext: true });
    console.debug('Fetching next playtoken');
    // @ts-ignore
    return this.props.client
      .mutate<NextPlayTokenMutation>({ mutation: nextPlayTokenQuery, variables: { trackId } })
      .then(result => {
  ...
  private togglePause = () => {
    ...
    this.setState((state: PlayerContext) => ({ playing: !state.playing }));
  };
  ...
  private load = (playToken: PlayToken) => {
